...
def build_report_pdf():
    ...
    story.append(Paragraph("One key challenge in the project was balancing coursework simplicity with professional API design. To address this, the final implementation kept the core structure beginner-friendly while still introducing practical enhancements such as consistent error payloads, automated tests, and deployment support. Another challenge was organizing the Git history after code generation had already been completed. This was resolved by restructuring the repository history into multiple logical commits that better represent a typical development sequence.", styles["BodySmall"]))
...
def build_presentation():
    ...
    for text in ["8ca73e4 Initialize coursework API repository", "a06c6a7 Add database configuration and task schemas", "a7cf785 Implement CRUD task API endpoints", "2e79950 Add sample seed data for local demonstrations", "10a11b3 Add automated API tests with isolated database", "4816579 Document project setup and deployment workflow"]:
        p = tf.add_paragraph(); p.text = text; p.level = 1
    slide = prs.slides.add_slide(prs.slide_layouts[1])
